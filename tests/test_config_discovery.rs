use drclean::{discover_config, find_config_upward};
use std::fs;
...
fn test_find_config_upward_in_current_dir() {
    ...
    let config_path = temp_dir.path().join(".drclean.toml");
    fs::write(&config_path, "path = \".\"").unwrap();

    let result = find_config_upward(temp_dir.path(), ".drclean.toml");
...
fn test_find_config_upward_in_parent_dir() {
    ...
    let result = find_config_upward(&child, ".drclean.toml");
    assert_eq!(result, Some(config_path));
}
...
fn test_find_config_upward_ignores_directories() {
    ...
    let config_dir = temp_dir.path().join(".drclean.toml");
